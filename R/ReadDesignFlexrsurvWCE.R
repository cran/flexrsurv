ReadDesignFlexrsurvWCEI<-function(modframe,
		Terms,
		data=parent.frame(),
		rate=NULL,
		Spline_t0,
		...
		contrasts=NULL){
	# extract X0, X and Z and W components of a flexrsurv formula
...
#print("ReadDesignFlexrsurvWCEI")
	...
	WCEIVars <- attr(Terms, "specials")$WCEI
	if( !is.null( WCEIVars )) {
		# get the indexes of the terms
		WCEITerms <-  variable2term(WCEIVars, Terms)
	} else {
		WCEITerms <-  NULL
	}
	
	if(length(c(NLLTerms, NPHTerms, NPHNLLTerms, WCEIVars))>0){
		LinVars <- (1:(length(AllVariables)))[-c(1,NLLVars, NPHVars, NPHNLLVars, WCEIVars, attr(Terms, "offset"))  ]
		LinTerms <- (1:(length(attr(Terms, "term.labels"))))[-c(NLLTerms, NPHTerms, NPHNLLTerms, WCEITerms)]
	...
	is.LIN <- is.NLL <- is.NPH <- is.NPHNLL <- is.NPHNLLMult <- is.NPHNLLAdd <- is.WCEI <- rep(FALSE, length( attr(Terms, "term.labels")))
	names(is.LIN) <- names(is.NLL) <- names(is.NPH) <- names(is.NPHNLL) <- names(is.NPHNLLMult) <- names(is.NPHNLLAdd)  <- names(is.WCEI) <- attr(Terms, "term.labels")
	...
	if(length(WCEITerms)>0) {
		is.WCEI[WCEITerms] <- TRUE
	}
...
# replace NPHNLL by NPH of additives NPHNLL terms
		...
		TermsW <- keep.terms(Terms, sort( WCEITerms) , keep.intercept = FALSE)
	} else {
		TermsW <- NULL
	}
	TermsWCEI <- TermsW 
...
# in W WCEI vars
	if (is.null(TermsW)){
		...
		# matrix of true NPH effets (W matrix in the fitter)
		# raw variables, splines bases are not expanded
		WVars <- all_specials_vars(TermsW,
				specials=c("WCEI"),
				unique=TRUE, order="formula")
		Spline_WT <- get_TimeSplinebasis(TermsW,
				data=data,
				specials=c("WCEI"),
				all.vars.func=all_specials_vars, 
				unique=TRUE, order="formula")
		inpW <- parse(text = paste("list(", paste(WVars, collapse = ","), ")"))
		W <- as.data.frame(eval(inpW, envir=data))
		Wi <- model.matrix(TermsW,data=data, contrasts.args=contrasts)
		
		df.TermsW <- table(attr(Wi, "assign"))
		paramW.fin <- cumsum(df.TermsW) 
		paramW.deb <- c(1, paramW.fin[-length(paramW.fin)]+1)
...
# nb of coefficients
	ncoef   <- df.T0 + sum(df.TermsX0) + sum(df.TermsX) + sum(df.TermsZ) + sum(df.TermsZT) + sum(df.TermsW)
...
#print(WCEIVars)
#
#print(cbind( is.LIN , is.NLL , is.NPH , is.NPHNLL , is.NPHNLLMult , is.NPHNLLAdd , is.WCEI ))
#print(cbind(is.X0, is.X, is.Z, is.W))
#
#  if (!is.null(TermsLin)){
#    print("TermsLin")
#    print(TermsLin)
...
#    print(dim(W))
#    print(dimnames(W))
#    print(attr(Wi, "assign"))
#  }
#
#  nterms0 <- length(df.TermsX0) + length(df.TermsX) + length(df.TermsZ)  + length(df.TermsW) 
...
# df by terms
	...
	if(!is.null(Spline_t0)){
		...
		term2param <- list(gamma0=1:df.T0, alpha0=NULL, beta0=NULL, alpha=NULL, beta=NULL, eta0=NULL)
	} else {
		# no baseline hazard
		# list to pass from vector of coefficients to the list used by flexrsurv.**.fit
		# param$xx = coef[term2param$xx]
...
# beta(T) term
			...
			if( is.W[it] ){
				# term it is WCEI
				df.Terms[it] <- df.Terms[it] + df.TermsW[num.TermsW[it]]
				param2term[count0 + (1:df.TermsW[num.TermsW[it]])] <- ngamma0 + nalpha0 + nbeta0 + nalpha + nbeta + paramW.deb[num.TermsW[it]]:paramW.fin[num.TermsW[it]]
				term2param$eta0=c(term2param$eta0, count0 + (1:df.TermsW[num.TermsW[it]]))
			}
...
# vector of length nterms =1 if Terms[i] is in X0 reps X, restp Z
					...
					num.TermsW=num.TermsW,          #              WCEI   effects in the formula (vectors of lenghth lenght(TermsW)
