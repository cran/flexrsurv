gr_cumhaz_flexrsurv_GA0B0AB_bh<-function(allparam, var,
		Y, X0, X, Z, 
		step, Nstep, 
	...
	if (debug) cat("# computing gradient of the cumulative hazard: gr_cumhaz_flexrsurv_GA0B0AB_bh\n")
...
#  traiter les Intercept_t_NPH
	...
	if (nZ) { 
		...
		for(iZ in 1:nZ){
			...
			dLdbeta <- cbind(dLdbeta, Intb[,-1, drop=FALSE] * Zalpha)
		}
	}
	else {
		dLdalpha <- NULL
