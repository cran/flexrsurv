...
predictCumulativeHazard.flexrsurv <- function(object, newdata = NULL,
                                    ...
                                    se.fit = FALSE, 
                                    na.action = na.pass, ...){
# predct cumulative hazard from 0 to time  
...
# numerical_integration_method 
  ...
  # computes steps for time integtration
    ...
    # extract components
    des <- ReadDesignFlexrsurv(Terms=Terms, modframe=m, data=newdata, rate=NULL, Spline_t0=Spline_t0)
...
# get coefficients in order baseline hazard | linear & non-linear effects | non Prop efefct | nonlin_nonprop effects 
    ...
    if(bhlink=="log"){
      if(dim(Y)[2] == 2){
        Predictors <- .computeCumulativeHazard_GA0B0AB(GA0B0AB=alltheparameters,
                                                 Y=Y, X0=X0, X=X, Z=Z,
        ...
        Predictors <- .computeCumulativeHazard_fromto_GA0B0AB(GA0B0AB=alltheparameters,
                                                 Y=Y, X0=X0, X=X, Z=Z,
                                                 step=step, Nstep=Nstep, 
                                                 intTD=intTD, intweightsfunc=intweightsfunc,
                                                 nT0basis=nT0basis,
    ...
    if (do.se.fit) {
      # dim(Y) managed in .computeStdErrorCumulativeHazard_GA0B0AB
        stdErrorPredictors <- .computeStdErrorCumulativeHazard_GA0B0AB(GA0B0AB=alltheparameters,
                                                                       var=object$var,
                                                                       Y=Y, X0=X0, X=X, Z=Z, 
                                                                       step=step, Nstep=Nstep, 
                                                                       intTD=intTD, intweightsfunc=intweightsfunc,
    ...
    # bhlink == "identity"
    if(dim(Y)[2] == 2){
      Predictors <- .computeCumulativeHazard_GA0B0AB_bh(GA0B0AB=alltheparameters,
                                                 Y=Y, X0=X0, X=X, Z=Z,
                                                 step=step, Nstep=Nstep, 
                                                 intTD=intTD, intweightsfunc=intweightsfunc,
                                                 nT0basis=nT0basis,
      ...
      Predictors <- .computeCumulativeHazard_fromto_GA0B0AB_bh(GA0B0AB=alltheparameters,
                                                 Y=Y, X0=X0, X=X, Z=Z,
                                                 step=step, Nstep=Nstep, 
                                                 intTD=intTD, intweightsfunc=intweightsfunc,
                                                 nT0basis=nT0basis,
    ...
    if (do.se.fit) {
      ...
      stdErrorPredictors <- .computeStdErrorCumulativeHazard_GA0B0AB_bh(GA0B0AB=alltheparameters,
                                                                           var=object$var,
                                                                           Y=Y, X0=X0, X=X, Z=Z, 
                                                                           step=step, Nstep=Nstep, 
                                                                           intTD=intTD, intweightsfunc=intweightsfunc,
