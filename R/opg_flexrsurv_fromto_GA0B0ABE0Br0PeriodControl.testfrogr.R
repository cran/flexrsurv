opgtestgr_ll_flexrsurv_fromto_GA0B0ABE0Br0PeriodControl<-function(allparam,
		Y, X0, X, Z, W,
		BX0,
...
# computes intermediates
		...
		if(is.null(Spline_B)){
			if( nBX0){
				...
				modified_cumratebyPcontrol <- log((1 + exp( expected_logit_end_byperiodcontrol))/(1 + exp(expected_logit_enter_byperiodcontrol))) * BPHtermcontrol
...
# by period
			...
			if(min(modified_ratecontrol)<0){
				print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   modified ratecontrol <0")
				print(summary(cbind(expected_ratecontrol , evalderivbrasscontrol, modified_ratecontrol)))
				print(brass0)
			}
		}
...
#^parameters of the  correction of th elife table
	...
	if(is.null(Spline_B)){
		...
		if(min(modified_rate)<0){
			print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   modified rate exposed <0")
			print(summary(cbind(expected_rate , evalderivbrass, modified_rate)))
			print(brass0)
		}
	}
