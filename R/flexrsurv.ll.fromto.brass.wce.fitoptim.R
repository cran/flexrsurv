flexrsurv.ll.fromto.brass0.wce.fitoptim<-function (X0, X, Z, Y,  W,
		BX0, 
		Id, FirstId,  
		isEnter,
		isEnd,
...
# NLL, NPH, NPHNLL and WCE effects
	...
	if( vartype == "opg"){
		stop("flexrsurv.ll.fromto.brass.wce.fit(): vartype == 'opg' not implemented for WCE effect")
	}
...
#    nTbasis <- 0 already done with beta0
	...
	if(!is.null(BX0)){
		...
		if(is.matrix(BX0)) {
			...
			stop("error flexrsurv.ll.fromto.brass.wce.fit(): wrong type of X0") 
		}
		nbalpha0<-nBX0
		...
		if (nW ){
			if (!is.null(init$eta0)) {
				if (length(init$eta0) != neta0){ 
					stop("flexrsurv.ll.fromto.brass.wce.fit(): Wrong length for initial values for eta0")
				}
				else {
					initeta0 <- init$eta0
					do.init.eta0 <- FALSE
		...
		if (nbrass0) {
			if (!is.null(init$brass0)) {
				if (length(init$brass0) != nbrass0){ 
					stop("flexrsurv.ll.fromto.brass.wce.fit(): Wrong length for initial values for brass0")
				}
				else {
		...
		if (nBX0) {
			if (!is.null(init$balpha0)) {
				if (length(init$balpha0) != nbalpha0){ 
					stop("flexrsurv.ll.fromto.brass.wce.fit(): Wrong length for initial values for balpha0")
				}
				else {
...
# end if (!missing(init) && !is.null(init)) {
	...
	if(method$int_meth == "CAV_SIM"){
		...
		stop("flexrsurv.ll.fromto.brass.wce.fit(): unknown method$int_meth")
	}
...
# objective, gradiant functions
	if( bhlink == "log"){
		ll_function    <- ll_flexrsurv_fromto_GA0B0ABE0Br0Control
		...
		gr_function    <- gr_ll_flexrsurv_fromto_GA0B0ABE0Br0Control
#    opgFunction    <- opg_flexrsurv_fromto_GA0B0ABE0Br0Control
#    gr_function    <- NULL
		opgFunction    <- NULL
	}
	else {
		stop("flexrsurv.ll.fromto.brass0.wce.fitoptim(): bhlink = 'identity' not implemented for WCE effect")
...
# end   if(do.init)
	...
	lowerGA0B0ABE0Br0 <- c(rep(bmin, length(c(gamma0, alpha0, beta0, alpha, beta, eta0))), c(-10, rep(1/10, length(brass0)-1)), rep(bmin, length(balpha0)))
	upperGA0B0ABE0Br0 <- rep(bmax, length(c(gamma0, alpha0, beta0, alpha, beta, eta0, brass0, balpha0)))
	...
	# constraints on brass parameters : brass0[1] unconstraints
	# brass0[-1] >= 0
	ui <- diag(nbrass0-1)
	ui <- cbind(matrix(0, ncol = length(initGA0B0ABE0Br0) - nbrass0 + 1, nrow=nbrass0-1), ui)
...
# numericNHessian in package maxLik
	...
	linearPredictors <- .computeLinearPredictor_GA0B0ABE0(GA0B0ABE0=fit$par,
			Y=Y, X0=X0, X=X, Z=Z, W=W,
			Id=Id, FirstId=FirstId,
			nT0basis=nT0basis,
			Spline_t0=Spline_t0, Intercept_t0=Intercept_t0,
...
#  # computes the cumulative exxcess hazard ??  
	cumulativeHazard <- .computeCumulativeHazard_fromto_GA0B0ABE0(GA0B0ABE0=fit$par,
			Y=Y, X0=X0, X=X, Z=Z, W=W,
			Id=Id, FirstId=FirstId,
			step=step, Nstep=Nstep, 
			intTD=intTD, intweightsfunc=intweightsfunc,
