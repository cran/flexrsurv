ll_flexrsurv_fromto_GA0B0ABE0Br0Control<-function(allparam,
		Y, X0, X, Z, W,
		BX0,
...
#      llcontrol <- crossprod(eventtermcontrol, weightscontrol)  - crossprod(modified_cumratecontrol, weightscontrol)
			llcontrol <- crossprod(eventtermcontrol  - modified_cumratecontrol, weightscontrol)
		}
		else {
# code equivalent to the one of the periode approach 
#      llcontrol <- sum( eventtermcontrol) - sum(modified_cumratecontrol )
...
#    print(summary(modified_cumrate))
	...
	if (!is.null(weights)) {
		if( nX0){
# code equivalent to the one of the periode approach 
#      llexposed <- crossprod(eventterm - PHterm * NPHterm , weights) - crossprod(modified_cumrate, weights)
			llexposed <- crossprod(eventterm - modified_cumrate - PHterm * NPHterm , weights)
		} else {
# code equivalent to the one of the periode approach 
#      llexposed <- crossprod(eventterm - NPHterm , weights) - crossprod(modified_cumrate, weights)
			llexposed <- crossprod(eventterm - modified_cumrate - NPHterm , weights)
		}
	}
	else {
		if( nX0){
			llexposed <- sum( eventterm ) - sum( modified_cumrate ) - sum(PHterm * NPHterm )
		} else {
			llexposed <- sum( eventterm) - sum(modified_cumrate) - sum(NPHterm )
		}
	}
